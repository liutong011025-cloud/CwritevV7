...
Created with CWrite
  ...
  const handleApplyCorrection = (errorIndex: number) => {
    ...
    // 应用修正
    const before = currentLetter.substring(0, error.start)
    const after = currentLetter.substring(error.end)
    const newLetter = before + error.corrected + after

    setCurrentLetter(newLetter)

    // 更新错误列表中的索引（因为文本长度可能改变）
    const lengthDiff = error.corrected.length - (error.end - error.start)
  ...
  const renderHighlightedText = () => {
    ...
    sortedErrors.forEach((error, errorIndex) => {
      ...
      parts.push({
        ...
        isError: true,
        errorIndex: grammarErrors.indexOf(error),
      })
    ...
    const errorRendered = new Set<number>() // 跟踪已渲染修正建议的错误
    ...
    parts.forEach((part, partIndex) => {
      ...
      lines.forEach((line: string, lineIdx: number) => {
        ...
        if (part.isError && part.errorIndex !== undefined) {
          ...
          const isLastLineOfError = lineIdx === lines.length - 1 // 只在错误的最后一行显示修正建议
          const showCorrection = isLastLineOfError && !errorRendered.has(part.errorIndex)
          
          if (showCorrection) {
            errorRendered.add(part.errorIndex)
          }
          ...
          result.push(
            ...
            >
              ...
              {/* 错误详情提示 */}
              {isHovered && (
                <div className="absolute z-50 bottom-full left-0 mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg shadow-lg whitespace-nowrap max-w-xs">
                  <div className="font-bold mb-1">问题：{error.issue}</div>
                  <div className="text-gray-300">原文：{error.original}</div>
                  <div className="text-green-300">建议：{error.corrected}</div>
              ...
              {showCorrection && (
                <span className="ml-2 inline-flex items-center gap-1">
                  ...
                  <span
                    ...
                    onClick={() => handleApplyCorrection(part.errorIndex!)}
                  >
