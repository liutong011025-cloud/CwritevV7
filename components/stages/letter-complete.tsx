...
Created with CWrite
  ...
  const handleApplyCorrection = (errorIndex: number) => {
    ...
    // 检查原文本中错误前后的字符
    const charBeforeError = error.start > 0 ? currentLetter[error.start - 1] : ''
    const charAfterError = error.end < currentLetter.length ? currentLetter[error.end] : ''
    
    // 检查错误文本本身是否以空格开头或结尾
    const originalErrorText = currentLetter.substring(error.start, error.end)
    const errorStartsWithSpace = originalErrorText.trimStart().length < originalErrorText.length
    const errorEndsWithSpace = originalErrorText.trimEnd().length < originalErrorText.length
    
    // 构建修正后的文本，保留原文本中的空格
    let corrected = error.corrected.trim() // 移除AI返回的修正文本两端的空格
    
    // 如果原文本错误前有空格（且错误本身不以空格开头），修正后前面也要有空格
    if (charBeforeError === ' ' && !errorStartsWithSpace && !corrected.startsWith(' ')) {
      corrected = ' ' + corrected
    }
    // 如果原文本错误后有空格（且错误本身不以空格结尾），修正后后面也要有空格
    if (charAfterError === ' ' && !errorEndsWithSpace && !corrected.endsWith(' ')) {
      corrected = corrected + ' '
    }
