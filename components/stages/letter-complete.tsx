...
Created with CWrite
  ...
  const handleApplyCorrection = (errorIndex: number) => {
    ...
    // 扩展为完整单词
    const expanded = expandToFullWord(currentLetter, error.start, error.end)
  ...
  const renderHighlightedText = () => {
    ...
    sortedErrors.forEach((error, errorIndex) => {
      ...
      // 添加错误文本（已扩展为完整单词）
      // 找到原始错误索引
      const originalErrorIndex = grammarErrors.findIndex(e => 
        Math.abs(e.start - error.start) <= 1 && Math.abs(e.end - error.end) <= 1
      )
      parts.push({
        text: currentLetter.substring(error.start, error.end),
        isError: true,
        errorIndex: originalErrorIndex >= 0 ? originalErrorIndex : errorIndex,
      })
