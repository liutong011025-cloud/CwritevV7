...
export default function BookReviewComplete({
  ...
  const handleApplyCorrection = (errorIndex: number) => {
    ...
    const before = currentReview.substring(0, error.start)
    const after = currentReview.substring(error.end)
    
    // 检查原文本中错误前后的字符
    const charBeforeError = error.start > 0 ? currentReview[error.start - 1] : ''
    const charAfterError = error.end < currentReview.length ? currentReview[error.end] : ''
    
    // 检查错误文本本身是否以空格开头或结尾
    const originalErrorText = currentReview.substring(error.start, error.end)
