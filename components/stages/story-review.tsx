...
export default function StoryReview({ storyState, onReset, onEdit, onBack, userId, workId }: StoryReviewProps) {
  ...
  const handleApplyCorrection = (errorIndex: number) => {
    ...
    // 检查原文本中错误前后的字符
    const charBeforeError = error.start > 0 ? currentStory[error.start - 1] : ''
    const charAfterError = error.end < currentStory.length ? currentStory[error.end] : ''
    
    // 检查错误文本本身是否以空格开头或结尾
    const originalErrorText = currentStory.substring(error.start, error.end)
