...
3. CRITICAL: You MUST mark the COMPLETE word that contains the error. Do NOT mark only part of a word.
   - If a word is misspelled (e.g., "go" should be "goes"), mark the ENTIRE word "go" from start to end
   - If a word is split incorrectly (e.g., "re ading" should be "reading"), mark the ENTIRE split portion "re ading" including the space
   - If multiple words are wrong (e.g., "I are" should be "I am"), mark both words "I are"
   - NEVER mark only part of a word like "go" when the error is "go" (mark the whole word)
4. original MUST be the EXACT text from the original content at positions [start:end]
5. corrected is the suggested correction - ONLY the corrected word(s), NO leading or trailing spaces
...
8. If there are no errors, return an empty array: []
9. Return ONLY the JSON array, no other text before or after
